...
test('result of query builder starts with the endpoint', () => {
...
test('a new query has pagination params by default', () => {
	const queryBuilder = makeQueryBuilder();
	expect(queryBuilder.toString()).toBe('api/endpoint/?page%5Blimit%5D=50&page%5Boffset%5D=0');
});
...
test('params with the same name get overwritten', () => {
	const queryBuilder = makeQueryBuilder();
	queryBuilder.params({
		'test1': 'test1Value',
		'test2': 'test2Value',
...
test('includes adds the appropriate params', () => {
	const queryBuilder = makeQueryBuilder();
...
test('paginate adds the appropriate params', () => {
	const queryBuilder = makeQueryBuilder();
...
test('sort adds the appropriate params', () => {
	const queryBuilder = makeQueryBuilder();
...
test('group adds the appropriate params', () => {
	const queryBuilder = makeQueryBuilder();
